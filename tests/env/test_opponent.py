...
class TestScriptedOpponent:
    def test_always_attacks(self) -> None:
        opp = ScriptedOpponent()
        result = opp.act(None, None, None, None)  # type: ignore[arg-type]
...
class TestRandomOpponent:
    def test_respects_action_mask(self) -> None:
        ...
        actions = {opp.act(None, mask, None, None) for _ in range(100)}  # type: ignore[arg-type]
        assert Action.EVADE in actions
        assert Action.WILLPOWER_EVADE in actions
        assert Action.ATTACK not in actions
        assert Action.WILLPOWER_ATTACK not in actions
    ...
    def test_deterministic_seed(self) -> None:
        ...
        for _ in range(20):
            a1 = opp1.act(None, mask, None, None)  # type: ignore[arg-type]
            a2 = opp2.act(None, mask, None, None)  # type: ignore[arg-type]
            assert a1 == a2
    ...
    def test_with_real_objects(self) -> None:
        ...
        action = opp.act(obs, mask, hunter, vampire)
        assert isinstance(action, Action)

    def test_fallback_on_empty_mask(self) -> None:
        opp = RandomOpponent(rng=np.random.default_rng(0))
        mask = np.zeros(4, dtype=np.int8)
        action = opp.act(None, mask, None, None)  # type: ignore[arg-type]
